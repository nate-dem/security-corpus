...
**Immediate next GPU job:** [quality review and additional examples](../scripts/curation/GPU_REVIEW.md).
CPU input preparation is still running. V3 recovered 16/18 assistant-eligible
cases, but both models admit too much weak or damaged content. A source-only
32B reviewer and 300 additional diagnostic examples are ready; the partition
scoring driver is implemented. See [the findings](curation_v3_review.md). No user
labeling task is pending. Assistant references are not independent human evidence.
...
| YouTube classifier | V3 comparison reviewed; source-only critic and resumable partition scoring implemented | Run critic/additional diagnostics; finish input preparation; validate production configuration and selection |
...
Assistant review and the v3 comparison review are complete. The GPU quality-review
job can run alongside ongoing CPU preparation. Production GPU execution and
near-dedup assembly remain unfinished. No dependency reinstall or download is
