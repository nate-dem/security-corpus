...
wrong-model decisions, null provenance, and mixed model/prompt/task configurations.
Pass `--expected-task qa --expected-prompt-version qwen-qa-v3-all-records-head-tail`
to bind the QA audit to the reviewed task and prompt as well.

Resuming retries parse failures and skips successful decisions. Merging keeps a
successful retry ahead of a failed attempt and refuses conflicting successful
keep/drop results or mixed inference settings. Changed scorer/prompt code,
source selection, inference settings, or runtime versions require a new shard
output directory. Keep all shard `run-config.json` files with the decisions.
Resume also checks a fingerprint of the exact input file bytes, including
prompt metadata, and explicitly pins the vLLM tokenizer revision.

Partial array retries must keep the original shard modulus (100 for QA, 24 for
citations), independent of how many indices are resubmitted. The templates now
do this explicitly. QA defaults to four concurrent jobs and a four-hour limit.
...
- expands includes only in active TeX, preserving code examples and repeated includes;
- writes content and status atomically.

The current LaTeX version is `latex-v3`. Missing status files and `latex-v2`
outputs are rejected. The normalizer also records the input archive SHA-256 and
checks it when resuming. PDF normalization selects `main.txt` even if stale
`main.tex` files exist. Download and metadata failures remain eligible for retry.

Use the union of the cs.CR restart IDs and the current accepted citation IDs as
`python scripts/ingest.py arxiv --id-file SELECTED_IDS`. This prevents cached
papers from an older citation selection entering the rebuilt corpus. Account
for every selected ID with an emitted record or a reviewed extraction failure.
...
includes per-file SHA-256 checksums and source totals. A diagnostic run with
`--skip-token-check` does not satisfy the token-count gate. Exact-content
duplicates are reported, without imposing a duplicate precedence policy.
