...
def test_normalize_weakness_returns_valid_normalized_data():
...
def test_category_and_view_normalize_successfully():
    ...
    assert isinstance(cat_result, NormalizedData)
    ...
    assert isinstance(view_result, NormalizedData)
...
def test_normalize_leaves_vuln_fields_empty():
    """CWE records should not populate severity, cvss_score, or cwe_ids."""
    record = _load_record(FIXTURES / "sample_weakness.xml", "Weakness", "Weakness")
    result = MitreCweConnector().normalize(record)
