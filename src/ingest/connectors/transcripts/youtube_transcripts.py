...
# train-* differ slightly — e.g. cctube_0 lacks source_language).
# iter_records() intersects _GATE_COLS with each file's actual schema before
# reading, so a missing column is skipped rather than raising an error.
_GATE_COLS = [
    "video_id",
    "channel_id",
    "channel",
...
def _is_english(record: dict) -> bool:
    ...
    1. transcription_language matches a valid BCP-47 English code (en, en-GB, …).
    2. transcription_language absent AND source_language='en'.
    3. transcription_language absent/garbage AND language_id_method='metadata'
       with original_language='en' (catches 'en-GHEw9DUond8'-style corruption).
    """
...
class YouTubeTranscriptsConnector:
    ...
    def iter_records(
        ...
        *,
        ...
        Two-pass column projection: pass 1 reads only _GATE_COLS (no transcript
        text); pass 2 fetches "text" only for row groups with passing rows.
        """
        ...
        for parquet_file in sorted(path.glob("cctube_*.parquet")):
            ...
            # Intersect with available columns — cctube_* and train-* shards
            # differ slightly; reading only available columns avoids ArrowInvalid.
            available_cols = [c for c in _GATE_COLS if c in set(pf.schema_arrow.names)]
